...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register(new URL('sw.js?v=67', window.location.href), { scope: './' }).catch(() => {});
  });
}
...
function syncLineupFromDom(container) {
  S.lineup = Array.from(container.querySelectorAll('.lineup-item'))
    .map(item => item.dataset.pid)
    .filter(Boolean);
}

function refreshLineupSlots(container) {
  container.querySelectorAll('.lineup-item').forEach((item, i) => {
    const slot = item.querySelector('.lineup-slot');
    if (slot) slot.textContent = i + 1;
  });
}

function getLineupDragAfterElement(container, y) {
  const items = [...container.querySelectorAll('.lineup-item:not(.dragging)')];
...
function setupDragItem(el, container) {
  ...
  el.addEventListener('pointermove', e => {
    if (!pointerDrag || pointerDrag.el !== el || pointerDrag.pointerId !== e.pointerId) return;
    const deltaY = Math.abs(e.clientY - pointerDrag.startY);
    if (!pointerDrag.dragging && deltaY < 8) return;
    e.preventDefault();
    ...
    const after = getLineupDragAfterElement(container, e.clientY);
    if (after) container.insertBefore(el, after);
    else container.appendChild(el);
    refreshLineupSlots(container);
  });
