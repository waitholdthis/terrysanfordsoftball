...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register(new URL('sw.js?v=73', window.location.href), { scope: './' }).catch(() => {});
  });
}
...
function buildExportData() {
  ...
  const totalInnings = Math.max(9, g.totalInnings || 9);
  const hitResults = ['1B','2B','3B','HR'];

  const lineup = S.lineup.map((pid, i) => {
    const p   = playerById(pid);
    const abs = g.atBats.filter(ab => ab.playerId === pid);
    const st  = computePlayerStats(abs);
    const runs = abs.filter(ab => ab.result === 'HR').length;
    return {
      num: i + 1,
      name: p?.name || '—',
      pos: p?.pos || '?',
      ...
      bb: st.bb,
      k: st.k,
      avg: st.ab > 0 ? st.avg.toFixed(3).replace(/^0/, '') : '.000',
    };
  });

  const resultBases = result => {
    if (result === '1B' || ['BB','HBP','E','FC'].includes(result)) return 1;
  ...
  const atBats = S.lineup.map(pid => Array.from({ length: totalInnings }, (_, inning) => {
    const abs = g.atBats.filter(a => a.playerId === pid && a.inning === inning + 1);
    if (!abs.length) return null;
    const label = abs.map(ab => ab.result).join('/');
    const bases = Math.max(...abs.map(ab => resultBases(ab.result)));
    const allOuts = abs.every(ab => ab.isOut || resultBases(ab.result) === 0);
    return { r: label, b: bases > 0 ? bases : (allOuts ? 0 : 1) };
  }));

  const usPerInning = Array(totalInnings).fill(0);
  const themPerInning = Array(totalInnings).fill(0);
  (g.inningLog || []).forEach((entry, idx) => {
    if (idx >= totalInnings) return;
    usPerInning[idx] = entry.us || 0;
    themPerInning[idx] = entry.them || 0;
  });
  if (g.inning <= totalInnings) {
    const loggedUs = usPerInning.reduce((sum, runs) => sum + runs, 0);
    const loggedThem = themPerInning.reduce((sum, runs) => sum + runs, 0);
    usPerInning[g.inning - 1] += Math.max(0, g.score.us - loggedUs);
    themPerInning[g.inning - 1] += Math.max(0, g.score.them - loggedThem);
  }

  const weAreHome = g.homeAway === 'home';
  const usHits = Array(totalInnings).fill(0);
  g.atBats.forEach(ab => {
    if (ab.inning <= totalInnings && hitResults.includes(ab.result)) usHits[ab.inning - 1]++;
  });
  const innings = weAreHome ? usPerInning : themPerInning;
  const awayInnings = weAreHome ? themPerInning : usPerInning;
  const hits = weAreHome ? usHits : Array(totalInnings).fill(0);
  const awayHits = weAreHome ? Array(totalInnings).fill(0) : usHits;

  return {
    home: g.homeAway === 'home' ? usName : theirName,
    away: g.homeAway === 'away' ? usName : theirName,
    date, field, totalInnings, innings, awayInnings, hits, awayHits,
    errors: Array(totalInnings).fill(0),
    awayErrors: Array(totalInnings).fill(0),
    ourTeamRole: weAreHome ? 'home' : 'away',
    lineup, atBats,
    pitching: [],
  };
}
